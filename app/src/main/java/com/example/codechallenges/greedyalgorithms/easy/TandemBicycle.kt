...
fun tandemBicycle(redShirtSpeeds: MutableList<Int>, blueShirtSpeeds: MutableList<Int>, fastest: Boolean): Int {
    ...
    for (i in redShirtSpeeds.indices) {
        val redShirt = redShirtSpeeds[i]
        val blueShirt = blueShirtSpeeds[i]
        speed += if (redShirt > blueShirt) redShirt else blueShirt
    }
    return speed
}

fun main() {
    println(tandemBicycle(mutableListOf(5, 5, 3, 9, 2), mutableListOf(3, 6, 7, 2, 1), true))
}

fun sampleOneTandemBicycle(redShirtSpeeds: MutableList<Int>, blueShirtSpeeds: MutableList<Int>, fastest: Boolean): Int {
    redShirtSpeeds.sort()
