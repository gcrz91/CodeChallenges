...
Note: You're allowed to mutate the string
...
fun minimumWaitingTime(queries: MutableList<Int>): Int {
    ...
    val sortedQueries = queries.sortedBy { it }
    var currentIndex = queries.size - 2
    var totalWait = 0

    while (currentIndex >= 0) {
        for (i in currentIndex downTo 0) {
            totalWait += sortedQueries[i]
        }
        currentIndex -= 1
    }
    return totalWait
...
// O(nlogn) time | O(1) - where n is the number of queries
fun sampleOneMinimumWaitingTime(queries: MutableList<Int>): Int {
    queries.sort()
